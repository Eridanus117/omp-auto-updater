...
## 当前行为
- 用户每次执行 `omp` 时，包装器先同步执行等价于 `omp update` 的自动更新，再用原始参数启动真实 OMP
- 自动更新和真实 OMP 的标准输入/输出、标准错误都直接透传，行为等价于依次执行 `omp update` 和原始 `omp` 命令
...
- 不在 OMP 正在运行时强制替换文件。
- 不为了验证而人为制造一个 OMP 新版本；真实验收以每次 `omp` 调用触发包装器为准。
