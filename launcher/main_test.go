...
func TestRunPassesArgumentsWithoutUpdating(t *testing.T) {
	directory := t.TempDir()
	marker := filepath.Join(directory, "updated.txt")
	...
	if got := run([]string{"--version"}); got != 29 {
		t.Fatalf("run() exit code = %d, want 29", got)
	}
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("update marker exists or could not be checked: %v", err)
	}
	args, err := os.ReadFile(argsFile)
	if err != nil {
		t.Fatalf("read forwarded args: %v", err)
	}
	if got := strings.TrimSpace(string(args)); got != "--version" {
		t.Fatalf("forwarded args = %q, want %q", got, "--version")
	}
}
